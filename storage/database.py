...
def init_db():
    ...
    for col, definition in [
        ...
        ("applied_at", "TEXT"), ("company_context", "TEXT"),
    ]:
        try:
...
def update_company_context(job_id: int, context: str):
    conn = get_connection()
    conn.execute("UPDATE jobs SET company_context=? WHERE id=?", (context, job_id))
    conn.commit()
    conn.close()
